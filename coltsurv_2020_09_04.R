...
pred.m.sex<-1 - predict(m2.sex, type = "resp",data.frame(age=age))
St.m.sex <- c(1, cumprod(pred.m.sex))
...
m2.sex <- glm(Fail ~ age + sex + age:sex, family = binomial(link=cloglog), data=dayobs)
...
########Cannot get the sex prediction to work...keep getting "Error in eval(predvars, data, env) : object 'sex' not found"
pred.m.sex<-1 - predict(m2.sex, type = "resp",data.frame(age=age)) 
St.m.sex <- c(1, cumprod(pred.m.sex))
age2<-c(18, age) # to allow plot of survival to age 19, given alive on day 18
